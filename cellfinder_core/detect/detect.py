...
class MainRunner:
    def __init__(self):
        ...
        Attributes
        ----------
        mp3d_filter_output_queue : multiprocessing.Queue
        bf_process : multiprocessing.Process
        """
        self.mp3d_filter_output_queue = MultiprocessingQueue()
        self.processes = []
        self.mp_3d_filter_queue = None
    ...
    def start(
        ...
        self.bf_process = multiprocessing.Process(
            target=mp_3d_filter.process, args=()
        )
...
def main(*args, **kwargs):
    return MainRunner().run(*args, **kwargs)
